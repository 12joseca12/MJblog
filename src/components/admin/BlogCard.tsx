...
export const BlogCard = ({ post, onEdit, onDelete, isFeatured, onToggleFeatured, className }: BlogCardProps) => {
    const { styles, theme } = useThemeStyles();
    const isDark = theme === "dark";

    // Logic to find the cover image from content blocks
    const getCoverImage = () => {
        // 1. Try to find a block explicitly typed as "banner"
        const bannerBlock = post.content?.find(b => b.type === "banner");
        if (bannerBlock?.value) return bannerBlock.value;

        // 2. Fallback to the first "image" block
        const firstImageBlock = post.content?.find(b => b.type === "image");
        if (firstImageBlock?.value) return firstImageBlock.value;

        // 3. Fallback to boolean false or empty string logic
        return null;
    };
